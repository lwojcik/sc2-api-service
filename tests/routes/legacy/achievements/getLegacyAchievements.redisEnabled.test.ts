...
describe('/legacy/achievements/:regionId (Redis enabled)', () => {
  const fastifyServer = fastify() as any;
  const url = '/legacy/achievements/1';
  const cacheSegment = 'legacyAchievements-1';
  const expectedTTL = getConfig(true).redis.ttl.legacy.achievements;

...
describe('/profile/ladderSummary/:regionId/:realmId/:profileId (Redis enabled)', () => {
  const fastifyServer = fastify() as any;
  const url = '/profile/ladderSummary/1/1/1';
  const cacheSegment = 'ladderSummary-1-1-1';
  const expectedTTL = getConfig(true).redis.ttl.ladderSummary;

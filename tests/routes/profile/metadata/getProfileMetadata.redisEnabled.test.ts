...
describe('/profile/metadata/:regionId/:realmId/:profileId (Redis enabled)', () => {
  const fastifyServer = fastify() as any;
  const url = '/profile/metadata/1/1/1';
  const cacheSegment = 'metadata-1-1-1';
  const expectedTTL = getConfig(true).redis.ttl.metadata;
